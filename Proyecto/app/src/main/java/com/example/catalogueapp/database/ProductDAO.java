...
public interface ProductDAO {
    ...
    @Query ("UPDATE product SET maquinariaName=:name, maquinariaDesc=:desc, maquinariaImg = :img WHERE maquinariaId = :id")
    public void updateWithoutRanking(String id, String name, String desc, String img);

    @Query("UPDATE product SET maquinariaRanking=:ranking WHERE maquinariaId LIKE :id")
    public void updateRanking(String id, int ranking);

    @Query("SELECT * FROM product")
    public LiveData<List<Product>> getAll();

    @Query("SELECT * FROM product WHERE maquinariaName LIKE :searchName")
